     ...
     (fn [resolve reject]
       ...
       (-> (p/let
            [artifact (get-latest-artifact-info repo)
             ...
             url (if-not artifact (do (emit "update-not-available" nil) (throw (js/Error. "no update available"))) (:url artifact))
             ...
             _ (when-not auto-download (throw (js/Error. "auto download is disabled")))
             ^js dl-res (fetch url)
